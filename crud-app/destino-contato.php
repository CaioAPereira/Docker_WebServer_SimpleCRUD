...
<div class="inicio">
    ...
    <div class="row">
        ...
        $sql = "insert into contato (nome, email, mensagem) values (?, ?, ?)"; // Esta apenas com dois argumentos, enviando 3º

        try {
            $stmt = $conn->prepare($sql);
            $result = $stmt->execute([$nome, $email, $msg]); // Senha não existe, inserido $msg
        } catch (Exception $e) {
            $result = false;
            $error = $e->getMessage();
        }

        if ($result == true) { // Estava !==
        ?>
